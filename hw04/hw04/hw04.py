...
def totals_tree(m):
    ...
    def append(node):
        if is_mobile(node):
            return total_weight(node), append(left(node)), append(right(node))
        elif is_arm(node):
            return append(end(node))
        elif is_planet(node):
            return [size(node)]
...
def replace_leaf(t, old, replacement):
    ...
    replace(new_tree, old, replacement)
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, password):
        ...
        if len(wrong_passwords) == 3:
            ...
            wrong_passwords.append(str(password))
